...
const GroupInfoModal = ({ open, onClose, group, users_count }) => {
    ...
    return (
        <div className="fixed inset-0 bg-slate-950/40 backdrop-blur-md flex items-center justify-center z-[200] p-4" onClick={onClose}>
            <div className="bg-white dark:bg-slate-900 w-full max-w-[360px] rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
                <div className={`h-20 ${PRIMARY_GRADIENT} relative`}>
                ...
                <div className="px-6 pb-6 -mt-8 text-center">
                    <div className="w-16 h-16 bg-white dark:bg-slate-800 rounded-2xl mx-auto shadow-xl flex items-center justify-center mb-3 border-[3px] border-white dark:border-slate-900">
                        <Users size={24} className="text-indigo-600" />
                    </div>
                    
                    <h3 className="text-lg font-black dark:text-white">{group.name}</h3>
                    ...
                    <div className="flex justify-center gap-5 mt-3">
                        <div className="flex flex-col items-center">
                            ...
                            <span className="flex items-center gap-1 text-xs font-bold text-slate-700 dark:text-slate-200">
                                {group.type === "public" ? <Globe size={12} className="text-emerald-500" /> : <Lock size={12} className="text-rose-500" />}
                                <span className="capitalize">{group.type}</span>
                            </span>
                        </div>
                        <div className="w-px h-6 bg-slate-200 dark:bg-slate-700 self-end" />
                        <div className="flex flex-col items-center">
                            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Members</span>
                            <span className="text-xs font-bold text-slate-700 dark:text-slate-200">{users_count}</span>
                        </div>
                    </div>

                    <div className="mt-6 space-y-3">
                        <div className="text-left p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-700/50">
                            <p className="text-[13px] text-slate-600 dark:text-slate-400 italic">"{group.description || "No mission statement yet."}"</p>
                        </div>
                        
                        <button 
                            onClick={handleCopyLink} 
                            className={`w-full flex items-center justify-center gap-2 py-3 rounded-xl font-bold text-[11px] uppercase tracking-wider transition-all ${
                                copied 
...
export const CreateGroupModal = ({ open, onClose, onSubmit }) => {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in">
            <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-[360px] p-6 border border-white dark:border-slate-800">
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-xl font-black dark:text-white flex items-center gap-2"><Sparkles size={18} className="text-indigo-500"/> New Hub</h2>
                    <button onClick={onClose} className="p-1.5 bg-slate-50 hover:bg-slate-100 dark:bg-slate-800 dark:hover:bg-slate-700 rounded-full text-slate-400 transition-colors"><X size={16} /></button>
                </div>
                <form onSubmit={handleSubmit} className="space-y-3">
                    <input type="text" placeholder="Community Name" className="w-full p-3 bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 rounded-xl text-sm dark:text-white outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all" onChange={(e) => setFormData({...formData, name: e.target.value})} required />
                    <textarea placeholder="What's this group about?" rows={3} className="w-full p-3 bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 rounded-xl text-sm dark:text-white outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all resize-none" onChange={(e) => setFormData({...formData, description: e.target.value})} />
                    <select className="w-full p-3 bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 rounded-xl text-sm dark:text-white outline-none font-medium cursor-pointer" onChange={(e) => setFormData({...formData, type: e.target.value})}>
                        <option value="public">🌍 Public (Anyone can join)</option>
                        <option value="private">🔒 Private (Invite only)</option>
                    </select>
                    <div className="pt-2">
                        <button type="submit" className={`w-full py-3 mt-1 text-white text-sm font-bold rounded-xl shadow-lg shadow-indigo-500/25 active:scale-95 transition-all ${PRIMARY_GRADIENT}`}>Create Community</button>
                    </div>
                </form>
            </div>
        </div>
            ...
            <aside className={`w-full lg:w-[320px] xl:w-[360px] flex flex-col h-full bg-white dark:bg-slate-900/50 backdrop-blur-xl border-r border-slate-200 dark:border-slate-800 transition-all z-30 shrink-0 ${selectedGroupId ? 'hidden lg:flex' : 'flex'}`}>
                ...
                <div className="p-5 pb-2 shrink-0">
                    <div className="flex justify-between items-center mb-5">
                        <h1 className="text-[20px] font-black tracking-tight dark:text-white">EduFlow Hubs</h1>
                    ...
                    {displayList.length === 0 ? (
                        ...
                        displayList.map((group) => (
                            <div 
                                ...
                                className={`group flex items-center p-2.5 rounded-2xl cursor-pointer transition-all hover:scale-[1.01] ${
            ...
            <main className={`flex-1 flex flex-col h-full relative bg-transparent ${!selectedGroupId ? 'hidden lg:flex' : 'flex'}`}>
                {selectedGroupId ? (
                            ...
                            <div className="max-w-4xl mx-auto relative group">
                                ...
                                <div className="relative flex items-center bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[22px] p-1.5 shadow-xl transition-all group-focus-within:border-indigo-400 dark:group-focus-within:border-indigo-500/50">
                                    ...
                                    <button 
                                        ...
                                        className={`w-9 h-9 rounded-[14px] flex items-center justify-center transition-all duration-300 ${
                                            !inputValue.trim() 
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center p-8 text-center pb-28">
                        ...
                        <h2 className="text-xl font-black text-slate-800 dark:text-white mb-2">Welcome to EduFlow Hubs</h2>
