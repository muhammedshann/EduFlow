    ...
    return (
        <div className="w-full max-w-6xl mx-auto pb-16">
            ...
            <div className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20 p-8 mt-8">
                ...
                {loading ? (
                    <div className="text-center py-16 text-slate-500 text-lg animate-pulse">
                            ...
                            >
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                                        <h3 className="text-slate-800 font-bold truncate pr-4" title={note.title}>
                                            {note.title}
                                        </h3>

                                        <span className="text-[10px] text-slate-500 flex items-center gap-1 shrink-0 whitespace-nowrap bg-white/50 px-2 py-1 rounded-full border border-slate-100 h-fit">
                                            <Clock className="w-3 h-3" />
                                            {new Date(note.created_at).toLocaleDateString()}
                                        </span>
                                    </div>

                                    <p className="text-slate-600 text-sm mt-2 leading-relaxed break-words line-clamp-3">
                                        {note.message}
                                    </p>
                                </div>
                            </div>
