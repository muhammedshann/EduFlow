...
import {
    ...
    Sparkles, Key, Timer, ArrowRight, X
} from "lucide-react";
                ...
                <div className="bg-white/95 backdrop-blur-2xl rounded-[2.5rem] shadow-2xl border border-white/50 overflow-hidden">
                    <div className="p-8 space-y-6">
                        ...
                        <div className="space-y-3">
                            <button
                                type="button" // Important: prevents form submission refreshes
                                onClick={(e) => {
                                    e.preventDefault();
                                    console.log("Verify button clicked with OTP:", otp);
                                    handleVerify();
                                }}
                                // Ensure the button is only enabled when the OTP is exactly the right length
...
export default function Settings() {
    ...
    const handleSendOTP = async () => {
        ...
        try {
            ...
            // await api.post('/accounts/generate-otp/', { email: formData.email });
    ...
    const handleVerifyAndSave = async () => {
        ...
        try {
            ...
            // await api.post('/accounts/verify-otp/', { email: formData.email, otp: otp });
            ...
            <div className="w-full max-w-4xl space-y-10">
                ...
                <div className="absolute top-8 right-8 bg-white shadow-md rounded-xl px-5 py-3 flex items-center gap-3 border border-gray-100 cursor-pointer" onClick={() => navigate('/wallet/')}>
                    ...
                    <div className="flex flex-col leading-tight">
                        ...
                        <span className="text-base font-semibold text-gray-800">${balance || 0}</span>
                    </div>
                </div>
                ...
                <section className="bg-white rounded-2xl p-8 shadow-sm flex gap-8 items-start">
                    <div className="w-16 h-16 rounded-full overflow-hidden cursor-pointer" onClick={() => ProfileInputRef.current.click()}>
                        {user?.profilePic ? <img src={user.profilePic} alt="Profile" className="w-full h-full object-cover" /> : <div className="bg-gradient-to-r from-purple-500 to-blue-500 flex items-center justify-center text-white text-lg font-bold w-full h-full">{user?.firstname?.[0]?.toUpperCase()}</div>}
                    </div>
                    <div className="flex-1 space-y-6">
                        <h2 className="flex items-center gap-2 text-lg font-semibold text-gray-700 mb-4"><User className="text-indigo-500 w-5 h-5" /> Profile Information</h2>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <div><label className="text-sm text-gray-500 mb-1 block">First Name</label><input name='first_name' value={formData.first_name} onChange={handleInputChange} className="w-full rounded-lg border border-gray-200 p-3" /></div>
                            <div><label className="text-sm text-gray-500 mb-1 block">Last Name</label><input name='last_name' value={formData.last_name} onChange={handleInputChange} className="w-full rounded-lg border border-gray-200 p-3" /></div>
                            <div className="sm:col-span-2"><label className="text-sm text-gray-500 mb-1 block">Username</label><input name='username' value={formData.username} onChange={handleInputChange} className="w-full rounded-lg border border-gray-200 p-3" /></div>
                            <div className="sm:col-span-2"><label className="text-sm text-gray-500 mb-1 block">Email Address</label><input name='email' value={formData.email} onChange={handleInputChange} type="email" className="w-full rounded-lg border border-gray-200 p-3" /></div>
                        </div>
                        <div className="flex gap-4">
                            ...
                            <button onClick={() => ProfileInputRef.current.click()} className="flex-1 border border-gray-300 py-2.5 rounded-lg text-base font-semibold hover:bg-gray-100 transition">Change Avatar</button>
                ...
                <section className="bg-white rounded-2xl p-8 shadow-sm">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 text-center gap-6 mb-8">
                        <div><p className="text-xs text-gray-400 uppercase font-semibold">Remaining</p><p className="mt-1 font-extrabold text-4xl text-indigo-600">{plan?.remaining_credits || 0}</p></div>
                        <div><p className="text-xs text-gray-400 uppercase font-semibold">Used</p><p className="mt-1 font-bold text-2xl text-gray-800">{plan?.used_credits || 0}</p></div>
                        <div><p className="text-xs text-gray-400 uppercase font-semibold">Last Top-up</p><p className="mt-1 font-bold text-2xl text-gray-800">{purchaseDate}</p></div>
                    </div>
                ...
                <section className="bg-white rounded-2xl p-8 shadow-sm">
                    ...
                    <div className="space-y-5 relative">
                        ...
                        <div><label className="block mb-2 font-semibold text-gray-700 text-sm">Current Password</label><input type={showPassword ? "text" : "password"} value={PasswordData.current_password} name="current_password" onChange={handlePasswordInput} className="w-full rounded-lg border border-gray-200 p-3" /></div>
                        <div><label className="block mb-2 font-semibold text-gray-700 text-sm">New Password</label><input type={showPassword ? "text" : "password"} value={PasswordData.new_password} name="new_password" onChange={handlePasswordInput} className="w-full rounded-lg border border-gray-200 p-3" /></div>
                        <div><label className="block mb-2 font-semibold text-gray-700 text-sm">Confirm Password</label><input type={showPassword ? "text" : "password"} value={PasswordData.confirm_password} name="confirm_password" onChange={handlePasswordInput} className="w-full rounded-lg border border-gray-200 p-3" /></div>
                    </div>
                ...
                <section className="bg-white rounded-2xl p-8 shadow-sm">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                        ...
                        <div className="bg-red-50 rounded-lg p-6 flex flex-col justify-between">
                            ...
                            <button onClick={HandleDeleteOpen} className="w-full mt-4 py-2 rounded-lg border border-red-500 text-red-600 font-semibold"><Trash2 className="inline-block w-4 h-4 mr-2" /> Delete Account</button>
                        </div>
                    </div>
                </section>
            </div>
