...
import { Mail, Lock, Key, ArrowRight, Sparkles } from "lucide-react";
...
function OtpPage() {
    ...
    const [step, setStep] = useState(isResetPassword ? 1 : 2); // Step control
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (step === 1) {
                ...
                const result = await dispatch(generateOtp(formData.email)).unwrap();
                setregister(false)
                setStep(2);
            } else if (step === 2) {
                console.log("Verify OTP:", formData.otp, "for email:", formData.email);
                try {
                    ...
                    setAlert('wrong otp','error')
                }
                // if (isResetPassword) setStep(3);
                // else showAlert("OTP verified successfully!");
            } else if (step === 3) {
                        ...
                        >
                            {isLoading ? (
                                ...
                                <>
                                    ...
                                    <span>{step === 3 ? "Reset Password" : "Next"}</span>
                                </>
                            )}
                        </button>
                    </div>
